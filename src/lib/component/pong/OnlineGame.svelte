<script lang="ts">
	...
	let prevSnapshot: { state: GameStateSnapshot; receivedAt: number } | null =
		null;
	let currSnapshot: { state: GameStateSnapshot; receivedAt: number } | null =
		null;
	...
	onMount(() => {
		...
		socket.on(
			...
			(data: { userId: number; timeout: number }) => {
						...
						const ballMoved =
							Math.abs(
								currSnapshot.state.ballX -
									prevSnapshot.state.ballX,
							) > 0.1 ||
							Math.abs(
								currSnapshot.state.ballY -
									prevSnapshot.state.ballY,
							) > 0.1;

						if (
							currSnapshot.state.phase === "playing" &&
							ballMoved
						) {
							stateToRender = extrapolateSnapshot(
								currSnapshot.state,
								overTimeSec,
							);
						} else {
	...
	function draw(ctx: CanvasRenderingContext2D, state: GameStateSnapshot) {
		...
		if (state.scoreFlash) {
			const flashOpacity = Math.max(
				0,
				(state.scoreFlashTimer / 0.5) * 0.15,
			);
	...
	function drawCountdown(
		...
		ctx.fillStyle =
			state.countdownDisplay === "GO!" ? "#ff6b9d" : "#ffffff";
		ctx.shadowColor =
			state.countdownDisplay === "GO!" ? "#ff6b9d" : "#ffffff";
		ctx.shadowBlur = 20;
